...
exports.getHighestEarner = async (req, res) => {
    try {
        const mentors = await Mentor.find({}).populate("user users");
        const meetRequests = JSON.parse(mentors.meetRequests);
        const numberOfMeetingsCompleted = meetRequests.filter(
            (meet) => meet.status === MEET_STATUS.COMPLETED
        ).length;

        const fees = mentors.chargePerHour;

        const data = {
            numberOfMeetingsCompleted,
            fees,
        };
